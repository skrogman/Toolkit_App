...
while ($true) {
    ...
    elseif ($Selection -match 'Launch Toolkit Enclave') {
        # Session already active, step directly past authentication handling
    ...
    # Process First-Time Authentication Decryption Block
    if (-not $LaunchTriggered) {
        ...
        try {
            ...
            Write-Host '[X] Invalid PIN.' -ForegroundColor Red
            Start-Sleep -Seconds 1
            continue
        }
    }
